...
//
// Usage:
//   node generate-site.js lumina        → Builds Lumina version
//   node generate-site.js alb-shining   → Builds Alb Shining version
//   node generate-site.js all           → Builds all configs
//
// The script:
...
`;
    ...
    console.log(`  ✅ Switched active config to: ${configName}`);
}
...
function copyDist(configName) {
    ...
    console.log(`  📁 Output saved to: dist-${configName}/`);
}
...
function main() {
    ...
    if (target === 'all') {
        console.log(`\n🏗️  Building ALL ${available.length} configs...\n`);
        ...
        console.log(`\n✅ All ${available.length} builds complete!\n`);
    } else if (available.includes(target)) {
        console.log(`\n━━━ Building: ${target} ━━━`);
        switchConfig(target);
        buildProject();
        copyDist(target);
        console.log(`\n✅ Build complete! Deploy dist-${target}/ to your hosting.\n`);
    } else {
